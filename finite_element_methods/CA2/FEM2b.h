...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  //EDIT
  if (node == 0) { // front lower-left node
    value = 0.125 * (1. - xi_1) * (1. - xi_2) * (1. - xi_3);
  }
  else if (node == 1) { // front lower-right node
    value = 0.125 * (1. + xi_1) * (1. - xi_2) * (1. - xi_3);
  }
  else if (node == 2) { // back lower-left node
    value = 0.125 * (1. - xi_1) * (1. + xi_2) * (1. - xi_3);
  }
  else if (node == 3) { // back lower-right node
    value = 0.125 * (1. + xi_1) * (1. + xi_2) * (1. - xi_3);
  }
  else if (node == 4) { // front upper-left node
    value = 0.125 * (1. - xi_1) * (1. - xi_2) * (1. + xi_3);
  }
  else if (node == 5) { // front upper-right node
    value = 0.125 * (1. + xi_1) * (1. - xi_2) * (1. + xi_3);
  }
  else if (node == 6) { // back upper-left node
    value = 0.125 * (1. - xi_1) * (1. + xi_2) * (1. + xi_3);
  }
  else if (node == 7) { // back upper-right node
    value = 0.125 * (1. + xi_1) * (1. + xi_2) * (1. + xi_3);
  }
  return value;
}
...
double c0 = 1./3., x_min = 0.,x_max = 0.04; //EDIT
  ...
  for(unsigned int globalNode=0; globalNode<totalNodes; globalNode++){
    if(nodeLocation[globalNode][0] == x_min){
      boundary_values[globalNode] = 300. * ((1. + c0 * (nodeLocation[globalNode][1] + nodeLocation[globalNode][2])));
 
    }
    else if(nodeLocation[globalNode][0] == x_max){
      boundary_values[globalNode] = 310. * (( 1. + c0 * (nodeLocation[globalNode][1] + nodeLocation[globalNode][2])));
    }
  }
}
                ...
                for(unsigned int j=0;j<dim;j++){
                  for(unsigned int I=0;I<dim;I++){
                    for(unsigned int J=0;J<dim;J++){
                      ...
                      Klocal(A, B) += kappa[i][j] * detJ * invJacob[I][i] * invJacob[J][j] * basis_gradient(A, quad_points[q1], quad_points[q2], quad_points[q3])[I] * basis_gradient(B, quad_points[q1], quad_points[q2], quad_points[q3])[J] * quad_weight[q1] * quad_weight[q2] * quad_weight[q3];
                    }
                  }
                }
              }
...
void FEM<dim>::output_results (){
	...
	sprintf(solutionName, "CA2b");

	std::string solutionFileName(solutionName); solutionFileName += ".vtk";
	
	std::cout << "Writing solution for Coding Assignment 2 to file : " << solutionFileName.c_str() << std::endl;
