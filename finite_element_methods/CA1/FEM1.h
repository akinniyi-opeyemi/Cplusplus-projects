...
:
fe(FE_Q<dim>(QIterated<1>(QTrapezoid<1>(),order)), dim), 
...
//linear 
  if(basisFunctionOrder==1){
    if(xi_at_node(node)==-1){
    	value = (1.-xi)/2.;
    }
    else if (xi_at_node(node)==1){
    	value = (1.+xi)/2.;
    }
  }
  //Quadratic
  ...
  //Cubic
    ...
    if(xi_at_node(node)==-1){
    	value = -9./16. * (xi - 1.)*(pow(xi,2) - 1./9.);
    }
    else if (xi_at_node(node)==1){
    	value = 9./16. * (xi + 1.)*(pow(xi,2) - 1./9.);
    }
    else if (xi_at_node(node)==-1./3){
    	value = 27./16. * (xi - 1./3)*(pow(xi,2) - 1.);
    }
    else if (xi_at_node(node)==1./3){
    	value = -27./16. * (xi + 1./3)*(pow(xi,2) - 1.);
    }
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  //Cubic
    ...
    if(xi_at_node(node)==-1){
    	value = (-27./16)*pow(xi,2) + 9*xi/8 + 1/16;
    }
    else if (xi_at_node(node)==1){
    	value = (27./16)*pow(xi,2) + 9*xi/8 - 1/16;
    }
    else if (xi_at_node(node)==-1./3){
    	value = (81./16)*pow(xi,2) - 9*xi/8 - 27/16;
    }
    else if (xi_at_node(node)==1./3){
    	value = (-81./16)*pow(xi,2) - 9*xi/8 + 27/16;
    }
...
void FEM<dim>::setup_system(){
  ...
  //quad_points[0] = -0.7746;
  //quad_points[1] = 0.;
  //quad_points[2] = 0.7746;

  //quad_weight[0] = 0.5556;
  //quad_weight[1] = 0.8889;
  //quad_weight[2] = 0.5556;//EDIT
  //quad_weight[1] = 1.;
  //Just some notes...

   // Weight for fifth point
...
quad_weight[4] = 0.2369268851;
 ...
 // quad_points[0] = -0.9324695142;
 // quad_points[1] = -0.6612093865;
//quad_points[2] = -0.2386191861;
//quad_points[3] =  0.2386191861;
//quad_points[4] =  0.6612093865;
//quad_points[5] =  0.9324695142;

//quad_weight[0] = 0.1713244924;
//quad_weight[1] = 0.3607615730;
//quad_weight[2] = 0.4679139346;
//quad_weight[3] = 0.4679139346;
//quad_weight[4] = 0.3607615730;
//quad_weight[5] = 0.1713244924;
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q=0; q<quadRule; q++){
      ...
      if (prob == 1){
      u_exact = g1 - (f_bar*pow(x,3)/(6.*E)) + (x*(f_bar*pow(L,3) - 6*E*g1 + 6*E*g2))/(6.*E*L);
      }
      else if (prob == 2){
      u_exact = g1 - (f_bar*pow(x,3)/(6.*E)) + (x*(Area*f_bar*pow(L,2) + 2*h))/(2.*Area*E);
      }
      else if (prob == 3){
      u_exact = g1 + x*(f_cap*pow(x,3) + (f_cap*pow(L,4) - 12*E*g1 + 12*E*g2)/L - 2*L*f_cap*pow(x,2))/(12.*E);
      }
      else if (prob == 4){
      u_exact = g1 + x*(f_cap*pow(x,3) + 2*(Area*f_cap*pow(L,3) + 6*h)/Area - 2*L*f_cap*pow(x,2))/(12.*E);
      }
			l2norm += pow((u_exact - u_h),2) * quad_weight[q] * h_e / 2.;
    }
  }
  return sqrt(l2norm);
}
